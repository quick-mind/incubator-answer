...
import (
	"net/url"
	"github.com/apache/incubator-answer/internal/base/chatToken"
...
func (uc *UserController) UserChatToken(ctx *gin.Context) {
	token := middleware.ExtractToken(ctx)
	if len(token) == 0 {
		handler.HandleResponse(ctx, nil, nil)
		return
	...
	resp, err := uc.userService.GetUserInfoByUserID(ctx, token, userInfo.UserID)
	customerInfo := chatToken.CustomerInfo{
		Platform:   "answer",
		UserID:     resp.ID,
		Username:   resp.Username,
		Email:      resp.EMail,
	}
	jwt_token, err := chatToken.GenerateJWT(customerInfo)
	handler.HandleResponse(ctx, err, jwt_token)
}